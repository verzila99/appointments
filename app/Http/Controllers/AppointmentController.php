...
class AppointmentController extends Controller
{
    ...
    #[ArrayShape(['appointments' => "array", 'instructors' => "array"])]
    ...
    public function store(Request $request)
    {
        ...
        if (Appointment:: where('user_id', auth()->user()->id)
        ...
        if (Appointment:: where('user_id', auth()->user()->id)
                       ->whereDay('start_time', Carbon::create($validated['time'])->day)
                       ->count() > 2
        ) {
    ...
    public function destroy($id): \Illuminate\Http\Response|int|\Illuminate\Contracts\Foundation\Application|\Illuminate\Contracts\Routing\ResponseFactory
    {
        if (is_numeric($id) &&
